...
function RestaurantForm() {
  ...
  useEffect(() => {
    ...
    if (id) {
      fetch(`http://localhost:3000/api/v1/restaurants/${id}`)
        ...
        .then((data) => {
          if (data.status === "success") {
            const { name, description, stars, category, coordinates, park } =
              data.data.data;
            setFormData({ 
              name, 
              description, 
              ...
              category, 
              coordinates,
              park: park?._id || park // Handle populated or ID
            });
          }
          setLoading(false);
        })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.status === "success") {
        const restaurantId = data.data.data._id || id;
        ...
        if (photo) {
          ...
          if (!resPhoto.ok) {
            toast.error("Restaurante guardado, pero error al subir foto");
          }
        }

        toast.success(`Restaurante ${isEdit ? "actualizado" : "creado"} correctamente`);
        navigate(-1);
      } else {
    ...
    <div className="min-h-screen bg-slate-50 py-12 px-4 sm:px-6 lg:px-8 pt-24">
      <div className="max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl p-8">
        <h2 className="text-2xl font-bold text-gray-900 mb-6">
          {id ? "Editar Restaurante" : "Agregar Nuevo Restaurante"}
        </h2>
